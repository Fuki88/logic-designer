...
{
    public partial class Graf_editor : UserControl
    {
       ...
       // bool paint = false;
        ...
        public static Graf_editor form;
...
/*/////////////////////////////--\/--\/--\/--\/--///////////////////////////////////////*/
        ...
        public static DialogResult InputBox(string title, string promptText, ref string value)
        {
            ...
            for (int i = 1; i <= Graf_editor.form.StateFirst; i++)
            {
                strFirst += "9";
            }

            for (int i = 1; i <= Graf_editor.form.StateSecond; i++)
            {
                strSecond += "9";
            }
...
//toto bude nasa funkcia
        public void NacitajUzly()
        {
            ArrayList _Nodes = new ArrayList();
            
            Cleanup();
            Stream stream = File.Open("obvod_tmp.z5", FileMode.Open);
            BinaryFormatter bF = new BinaryFormatter();
            _Nodes = (ArrayList)bF.Deserialize(stream);

            ArrayList _Cons = new ArrayList();
            Stream stream1 = File.Open("arc_tmp.z5", FileMode.Open);
             ...
             foreach (PluginInterface.SavedCon conex in _Cons)
                ...
                foreach (PluginInterface.SavedCon conex in _Cons)
                {
                    foreach (Connection con in Connections)
                    {
                        ...
                        con.StartNode.Text = conex.StartNode;
                        con.EndNode.Text = conex.EndNode;
                            foreach(String n in con.StartNode.ConIN){
                                a++;
                            if (n == con.Name)  p = a; a=0;
                            
                            }
                              foreach(String n in con.StartNode.ConOut){
                                b++;
                            if (n == con.Name) q = b; b=0;
                            
                            }

                            PictureBox pic = new PictureBox();
                            PictureBox pic1 = new PictureBox();
                        StartConnection(con.StartNode, p,"",pic);
                            EndConnection(con.EndNode,q,false,pic1);
                        }
                    }
        ...
        private void názvySpojeníToolStripMenuItem_Click(object sender, EventArgs e)
        {
